...
class TextLayer:
    ...
    def all_embedding_distances(self, mouse_location):
        ...
        embedding_distances = [
            self.user_embedding_distance(mouse_location, text_coordinate)
            for text_coordinate in self.embeddings_coordinates_pca
        ]
        ...
        if len(index) > 0:
            normalized_embedding_distances = np.where(
                normalized_embedding_distances
                == normalized_embedding_distances[index[0]],
                normalized_embedding_distances,
                0,
            )
    ...
    def decomposition(self):
        ...
        embedding_coordinates_pca_normalized = scaler.fit_transform(
            embeddings_coordinates_pca
        )
    ...
    def create_inital_audio(self):
        ...
        for text_label in self.text_labels:
            spectral_image = self.pipe(
                ...
                0
            ]  # here we are just using the first image we should use clip to rank these
